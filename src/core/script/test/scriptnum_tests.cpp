...
BOOST_AUTO_TEST_SUITE(script_num_test)
...
static bool verify(const CScriptNum10 &bignum, const ScriptNum &scriptnum) {
    return bignum.getvch() == scriptnum.GetVector() &&
           bignum.getint() == scriptnum.GetValue();
}

static void CheckCreateVch(const int64_t &num) {
    CScriptNum10 bignum(num);
    ScriptNum scriptnum(num);
    BOOST_CHECK(verify(bignum, scriptnum));

    std::vector<uint8_t> vch = bignum.getvch();

    CScriptNum10 bignum2(bignum.getvch(), false);
    vch = scriptnum.GetVector();
    ScriptNum scriptnum2(scriptnum.GetVector(), false);
    BOOST_CHECK(verify(bignum2, scriptnum2));

    CScriptNum10 bignum3(scriptnum2.GetVector(), false);
    ScriptNum scriptnum3(bignum2.getvch(), false);
...
static void CheckCreateInt(const int64_t &num) {
    ...
    BOOST_CHECK(verify(bignum, scriptnum));
    BOOST_CHECK(
        verify(CScriptNum10(bignum.getint()), ScriptNum(scriptnum.GetValue())));
    BOOST_CHECK(
        verify(CScriptNum10(scriptnum.GetValue()), ScriptNum(bignum.getint())));
    BOOST_CHECK(verify(CScriptNum10(CScriptNum10(scriptnum.GetValue()).getint()),
                       ScriptNum(ScriptNum(bignum.getint()).GetValue())));
}

static void CheckAdd(const int64_t &num1, const int64_t &num2) {
    const CScriptNum10 bignum1(num1);
    const CScriptNum10 bignum2(num2);
    const ScriptNum scriptnum1(num1);
    const ScriptNum scriptnum2(num2);
    CScriptNum10 bignum3(num1);
    CScriptNum10 bignum4(num1);
    ScriptNum scriptnum3(num1);
    ScriptNum scriptnum4(num1);
...
static void RunCreate(const int64_t &num) {
    CheckCreateInt(num);
    ScriptNum scriptnum(num);
    if (scriptnum.GetVector().size() <= ScriptNum::MAXIMUM_ELEMENT_SIZE) {
        CheckCreateVch(num);
    } else {
...
static void CheckMinimalyEncode(std::vector<uint8_t> data,
    ...
    bool alreadyEncoded = ScriptNum::IsMinimallyEncoded(data, data.size());
    bool hasEncoded = ScriptNum::MinimallyEncode(data);
